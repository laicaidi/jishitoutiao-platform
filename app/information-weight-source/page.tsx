...
/**
 * 权重源数据实体类型定义
 * 严格对应数据库表 information_weight_source 字段
 */
interface InformationWeightSourceItem {
  weight_source_id: string | number; // 权重源id 主键
  bid: string;                  // 源id
...
export default function InformationWeightSourcePage() {
  // 获取全局公共数据源、分类数据源
  ...
  const [dataSource, setDataSource] = useState<InformationWeightSourceItem[]>([]);
  ...
  const [deleteRecord, setDeleteRecord] = useState<InformationWeightSourceItem | null>(null);
  ...
  const fetchList = async () => {
    try {
      ...
      const res = await getInformationWeightSourceApi(params);
   ...
   */
  const handleEditOpen = (record: InformationWeightSourceItem) => {
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const res = await addInformationWeightSourceApi(submitParams);
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const res = await updateInformationWeightSourceApi(values.weight_source_id, submitParams);
      const msg = res.data?.status?.message;
   ...
   * 打开删除确认弹窗
   */
  const handleDeleteOpen = (record: InformationWeightSourceItem) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const res = await deleteInformationWeightSourceApi(deleteRecord.weight_source_id);
      const msg = res.data?.status?.message;
   ...
   */
  const columns: TableProps<InformationWeightSourceItem>['columns'] = [
    {
      title: '序号',
      width: 70,
      align: 'center',
    ...
    { title: '权重源id', dataIndex: 'weight_source_id', width: 220 },
  ...
  return (
    <MainLayout>
      ...
      {/* 新增弹窗 */}
      <Modal open={addVisible} onCancel={() => setAddVisible(false)} title="新增权重源数据" footer={null} width={700}>
      ...
      {/* 修改弹窗 */}
      <Modal open={editVisible} onCancel={() => setEditVisible(false)} title="修改权重源数据" footer={null} width={700}>
        <Form form={editForm} layout="vertical" style={{ height: '70vh', overflowY: 'auto' }}>
          <Form.Item name="weight_source_id" hidden><Input /></Form.Item>
      ...
      >
        {deleteRecord && (
          <div style={{ padding: '10px 0' }}>
            <p><strong>权重源id:</strong> {deleteRecord.weight_source_id}</p>
