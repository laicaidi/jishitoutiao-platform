...
import {
  fetchGetAllInformationWeightSet,
  fetchAddInformationWeightSet,
  fetchDeleteInformationWeightSet,
...
import { UpdateInformationWeightSetComponent } from '../component/UpdateInformationWeightSetComponent';

const { Content } = Layout;

class InformationWeightSetContainer extends Component {
  constructor(props) {
    super(props);
    this.state = {
      addModalVisible: false,    // 新增权重设置对话框显示状态
      updateModalVisible: false    // 更新权重设置对话框显示状态
    }
    // 绑定回调方法
            ...
            payloadObj[field].push({
              ...
              render: (text, record) => {
                ...
                for (let key in dataObj) {
                  if (key === 'wkey') {
                      var msgWKey = dataObj[key];
                  } 
                  if (key === 'wvalue') {
                      var msgWValue = dataObj[key];
                  }  
                  if (key === 'sid') {        // 本条记录的sid
                      var sid = dataObj[key];
                  }
                ...
                function showUpdateModel() {    
                  // 通过回调方法，显示更新权重设置对话框，并传递代表本条数据的对象
  ...
  handleGet(keyword, pageNum) {
    ...
    if (keyword !== undefined && keyword !== null) {    // 1.1搜索框唤起
      dispatch(changeInformationWeightSetFiltrate(keyword));
    ...
    // 3.通过props中的值发起请求
    dispatch(fetchGetAllInformationWeightSet(keyword, pageNum));
  }
  ...
  handleAdd() {
    ...
    form.validateFields((err, values) => {
      ...
      // 提交新增请求
      dispatch(fetchAddInformationWeightSet(formData, this.addResult, this.handleGet));
    });
  }
  ...
  handleDelete(id) {
    const { dispatch } = this.props;
    // 提交删除请求
    dispatch(fetchDeleteInformationWeightSet(id, this.handleGet));
  }
  ...
  handleUpdate() {
    ...
    form.validateFields((err, values) => {
      ...
      // 提交更新请求
      dispatch(fetchUpdateInformationWeightSet(sid, formData, this.updateResult, this.handleGet));
    });
  }
  ...
  handleSearchInputChange(text) {
    ...
    dispatch(changeInformationWeightSetFiltrate(text));
  }
  ...
  render() {
    return (
        <Content style={ {margin: '24px 16px', padding: 24, background: '#fff', minHeight: 615} }>
            ...
            />
            <UpdateInformationWeightSetComponent
...
// 对属性进行限定
InformationWeightSetContainer.propTypes = {
...
function mapStateToProps(state) {
  return {
      payload: state.getInformationWeightSet.payload,    // 表格和分页渲染所需数据
      isFetching: state.getInformationWeightSet.isFetching,    // 获取权重设置数据加载态
      status: state.getInformationWeightSet.status,    // 获取权重设置数据返回结果
      receiveAt: state.getInformationWeightSet.receiveAt,    // 获取权重设置数据成功时间
      isCreating: state.addInformationWeightSet.isCreating,   // 新增权重设置加载态
      createStatus: state.addInformationWeightSet.status,    //  新增权重设置返回结果(包含提示信息)
      deleteStatus: state.deleteInformationWeightSet.status,    // 删除权重设置返回结果
      isUpdating: state.updateInformationWeightSet.isUpdating,   // 更新权重设置加载态
      updateStatus: state.updateInformationWeightSet.status,    // 更新权重设置返回结果
      height: state.updateScreen.height,   // 屏幕高(浏览器可视区域)
      keyword: state.changeInformationWeightSetFiltrate.keyword    // 搜索关键字
  }
}

export default connect(mapStateToProps)(InformationWeightSetContainer);