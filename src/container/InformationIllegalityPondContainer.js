...
import {
    fetchGetAllInformationIllegalityPond,
    fetchCleanInformationIllegalityPond,
    changeInformationIllegalityPondFiltrate
...
class InformationIllegalityPondContainer extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    handleGet(keyword, pageNum, bkey, ckey) {
        ...
        if (keyword !== null && keyword !== undefined) {        // 1.1从搜索框唤起
            // 更新props中的内容
            dispatch(changeInformationIllegalityPondFiltrate(keyword, this.props.bkey, this.props.ckey));
        ...
        // 5.通过props中的值发起请求
        dispatch(fetchGetAllInformationIllegalityPond(keyword, pageNum, bkey, ckey));
    }
    ...
    handleClean() {
        const { dispatch } = this.props;
        // 提交清空请求
        dispatch(fetchCleanInformationIllegalityPond(this.handleGet));
    }
    ...
    handleSourceChange(bkey) {
        ...
        dispatch(changeInformationIllegalityPondFiltrate(this.props.keyword, bkey, this.props.ckey));
    ...
    handleCategoryChange(ckey) {
        const { dispatch } = this.props;
        // 1.更新store中对应数据,其他字段使用现有值
        dispatch(changeInformationIllegalityPondFiltrate(this.props.keyword, this.props.bkey, ckey));
    ...
    handleSearchInputChange(text) {
        ...
        dispatch(changeInformationIllegalityPondFiltrate(text, this.props.bkey, this.props.ckey));
    }
    ...
    render() {
        ...
        if (redisData !== undefined && redisData !== null) {
            for (let key in redisData) {
                if (key === 'string:information_illegality_pond:batch') {
                ...
                <Tabs
                    ...
                    { categoryTabs.map(pane => <TabPane tab={ pane.tab } key={ pane.key } >
                        ...
                        <div style={ {marginTop: 24} }>
                            ...
                            <Button type='primary' style={ {float: 'right'} } onClick={ this.handleShowCleanModal }>清空滤非法池表</Button>
                        </div>
                        <TableComponent rowKey='illegality_pond_id' tableData={ this.props.payload } onGet={ this.handleGet } loading={ this.props.isFetching } /> 
                    </TabPane>) }
                </Tabs>
                <Modal title='清空滤非法池'
...
// 对属性进行限定
InformationIllegalityPondContainer.propTypes = {
...
function mapStateToProps(state) {
    return {
        payload: state.getInformationIllegalityPond.payload,    // 表格和分页渲染所需数据
        redisData: state.getInformationIllegalityPond.redisData,        // redis中的状态(status)和批次号(batch)数据
        isFetching: state.getInformationIllegalityPond.isFetching,    // 获取滤非法池数据加载态
        status: state.getInformationIllegalityPond.status,    // 获取滤非法池数据返回结果
        receiveAt: state.getInformationIllegalityPond.receiveAt,    // 获取滤非法池数据成功时间
        isCleaning: state.cleanInformationIllegalityPond.isCleaning,        // 清空加载态
        cleanStatus: state.cleanInformationIllegalityPond.status,    // 清空滤非法池返回结果
        ...
        keyword: state.changeInformationIllegalityPondFiltrate.keyword,    // 搜索关键字
        bkey: state.changeInformationIllegalityPondFiltrate.bkey,     // 源下拉列表选项
        ckey: state.changeInformationIllegalityPondFiltrate.ckey,     // 选中的类别标签
    }
}

export default connect(mapStateToProps)(InformationIllegalityPondContainer);