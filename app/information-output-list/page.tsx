...
/**
 * 资讯列表输出实体类型定义
 * 严格对应数据库表 information_output_list 字段
 */
interface InformationOutputListItem {
  output_list_id: string | number;    // 输出列表id
  bid: string;                  // 源id
...
export default function InformationOutputListPage() {
  // 获取全局公共数据源、分类数据源
  ...
  const [dataSource, setDataSource] = useState<InformationOutputListItem[]>([]);
  ...
  const [deleteRecord, setDeleteRecord] = useState<InformationOutputListItem | null>(null);
  ...
  const fetchList = async () => {
    try {
      ...
      const res = await getInformationOutputListApi(params);
   ...
   * 打开编辑弹窗
   */
  const handleEditOpen = (record: InformationOutputListItem) => {
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const res = await addInformationOutputListApi(submitParams);
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const res = await updateInformationOutputListApi(values.output_list_id, submitParams);
      const msg = res.data?.status?.message;
   ...
   * 打开删除确认弹窗
   */
  const handleDeleteOpen = (record: InformationOutputListItem) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const res = await deleteInformationOutputListApi(deleteRecord.output_list_id);
      const msg = res.data?.status?.message;
   ...
   */
  const columns: TableProps<InformationOutputListItem>['columns'] = [
    {
      title: '序号',
      width: 70,
      align: 'center',
    ...
    { title: '输出列表id', dataIndex: 'output_list_id', width: 220 },
  ...
  return (
    <MainLayout>
      ...
      {/* 新增弹窗 */}
      <Modal open={addVisible} onCancel={() => setAddVisible(false)} title="新增资讯列表源数据" footer={null} width={700}>
      ...
      {/* 修改弹窗 */}
      <Modal open={editVisible} onCancel={() => setEditVisible(false)} title="修改资讯列表源数据" footer={null} width={700}>
        <Form form={editForm} layout="vertical" style={{ height: '70vh', overflowY: 'auto' }}>
          <Form.Item name="output_list_id" hidden><Input /></Form.Item>
      ...
      >
        {deleteRecord && (
          <div style={{ padding: '10px 0' }}>
            <p><strong>输出列表id:</strong> {deleteRecord.output_list_id}</p>
