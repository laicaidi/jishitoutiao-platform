...
/**
 * 权重得分排序实体类型定义
 * 严格对应数据库表 information_weight_sort 字段
 */
interface InformationWeightSortItem {
  weight_sort_id: string | number;    // 得分表id
  bid: string;                  // 源id
...
export default function InformationWeightSortPage() {
  // 获取全局公共数据源、分类数据源
  ...
  const [dataSource, setDataSource] = useState<InformationWeightSortItem[]>([]);
  ...
  const [deleteRecord, setDeleteRecord] = useState<InformationWeightSortItem | null>(null);
  ...
  const fetchList = async () => {
    try {
      ...
      const res = await getInformationWeightSortApi(params);
   ...
   */
  const handleEditOpen = (record: InformationWeightSortItem) => {
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const res = await addInformationWeightSortApi(submitParams);
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const res = await updateInformationWeightSortApi(values.weight_sort_id, submitParams);
      const msg = res.data?.status?.message;
   ...
   * 打开删除确认弹窗
   */
  const handleDeleteOpen = (record: InformationWeightSortItem) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const res = await deleteInformationWeightSortApi(deleteRecord.weight_sort_id);
      const msg = res.data?.status?.message;
   ...
   */
  const columns: TableProps<InformationWeightSortItem>['columns'] = [
    {
      title: '序号',
      width: 70,
      align: 'center',
    ...
    { title: '得分表id', dataIndex: 'weight_sort_id', width: 220 },
  ...
  return (
    <MainLayout>
      ...
      {/* 新增弹窗 */}
      <Modal open={addVisible} onCancel={() => setAddVisible(false)} title="新增权重得分排序" footer={null} width={700}>
      ...
      {/* 修改弹窗 */}
      <Modal open={editVisible} onCancel={() => setEditVisible(false)} title="修改权重得分排序" footer={null} width={700}>
        <Form form={editForm} layout="vertical" style={{ height: '70vh', overflowY: 'auto' }}>
          <Form.Item name="weight_sort_id" hidden><Input /></Form.Item>
      ...
      >
        {deleteRecord && (
          <div style={{ padding: '10px 0' }}>
            <p><strong>得分表id:</strong> {deleteRecord.weight_sort_id}</p>
            <p><strong>资讯id:</strong> {deleteRecord.information_id}</p>
            <p><strong>标题:</strong> {deleteRecord.title}</p>
            <p><strong>爬虫id:</strong> {deleteRecord.crawler_id}</p>
