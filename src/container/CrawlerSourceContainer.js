...
class CrawlerSourceContainer extends Component {
  constructor(props) {
    super(props);
            ...
            payloadObj[field].push({
              ...
              render: (text, record) => {
                ...
                function showUpdateModel() {    
                  // 通过回调方法，显示更新爬虫源对话框，并传递代表本条数据的对象
  ...
  handleGet(keyword, pageNum) {
    ...
    // 3.通过props中的值发起请求
    dispatch(fetchGetAllCrawlerSource(keyword, pageNum));
  }
  ...
  handleSearchInputChange(text) {
    ...
    dispatch(changeCrawlerSourceFiltrate(text));
  }
...
// 对属性进行限定
CrawlerSourceContainer.propTypes = {
...
function mapStateToProps(state) {
  return {
      ...
      isFetching: state.getCrawlerSource.isFetching,    // 获取爬虫源数据加载态
      status: state.getCrawlerSource.status,    // 获取爬虫源数据返回结果
      receiveAt: state.getCrawlerSource.receiveAt,    // 获取爬虫源数据成功时间
      isCreating: state.addCrawlerSource.isCreating,   // 新增爬虫源加载态
      createStatus: state.addCrawlerSource.status,    //  新增爬虫源返回结果(包含提示信息)
      deleteStatus: state.deleteCrawlerSource.status,    // 删除爬虫源返回结果
      isUpdating: state.updateCrawlerSource.isUpdating,   // 更新爬虫源加载态
      updateStatus: state.updateCrawlerSource.status,    // 更新爬虫源返回结果
      height: state.updateScreen.height,   // 屏幕高(浏览器可视区域)
      keyword: state.changeCrawlerSourceFiltrate.keyword    // 搜索关键字
  }
}
...
*/
export default connect(mapStateToProps)(CrawlerSourceContainer);