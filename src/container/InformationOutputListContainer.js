...
import {
    fetchGetAllInformationOutputList,
    fetchDeleteInformationOutputList,
...
import { UpdateInformationOutputListComponent } from '../component/UpdateInformationOutputListComponent';

const { Content } = Layout;
const Option = Select.Option;
const TabPane = Tabs.TabPane;

class InformationOutputListContainer extends Component {
    constructor(props) {
        super(props);
        this.state = {
            updateModalVisible: false,    // 更新列表输出数据对话框显示状态
        }
        // 绑定回调方法
                            ...
                            render: (text, record) => {
                                ...
                                for (let key in dataObj) {
                                    ...
                                    if (key === 'output_list_id') {
                                        var outputListId = dataObj[key];     // 本条记录的列表输出数据id
                                    }
                                }
                                ...
                                function showUpdateModal() {
                                    // 通过回调方法，显示更新列表输出数据对话框，并传递代表本条数据的对象
                                    handleShowUpdateModal(dataObj);
                                }

                                function deleteConfirm() {
                                    // 通过回调方法删除列表输出数据
                                    handleDelete(outputListId);
                                }
    ...
    handleGet(keyword, pageNum, bkey, ckey) {
        ...
        // 5.通过props中的值发起请求
        dispatch(fetchGetAllInformationOutputList(keyword, pageNum, bkey, ckey));
    }
    ...
    handleSortChange(bkey) {
        ...
        dispatch(changeInformationOutputListFiltrate(this.props.keyword, bkey, this.props.ckey));
    ...
    handleCategoryChange(ckey) {
        const { dispatch } = this.props;
        // 1.更新store中对应数据,其他字段使用现有值
        dispatch(changeInformationOutputListFiltrate(this.props.keyword, this.props.bkey, ckey));
    ...
    handleSearchInputChange(text) {
        ...
        dispatch(changeInformationOutputListFiltrate(text, this.props.bkey, this.props.ckey));
    }
...
// 对属性进行限定
InformationOutputListContainer.propTypes = {
...
function mapStateToProps(state) {
    return {
        payload: state.getInformationOutputList.payload,    // 表格和分页渲染所需数据
        redisData: state.getInformationOutputList.redisData,        // redis中的状态(status)和批次号(batch)数据
        isFetching: state.getInformationOutputList.isFetching,    // 获取列表输出数据数据加载态
        status: state.getInformationOutputList.status,    // 获取列表输出数据数据返回结果
        receiveAt: state.getInformationOutputList.receiveAt,    // 获取列表输出数据数据成功时间
        deleteStatus: state.deleteInformationOutputList.status,    // 删除列表输出数据返回结果
        isUpdating: state.updateInformationOutputList.isUpdating,   // 更新列表输出数据加载态
        updateStatus: state.updateInformationOutputList.status,    // 更新列表输出数据返回结果
        height: state.updateScreen.height,   // 屏幕高(浏览器可视区域)
        sourceOption: state.getMeta.sourceOption,   // 列表输出数据源元数据
        categoryList: state.getMeta.categoryList,   // 类别元数据
        keyword: state.changeInformationOutputListFiltrate.keyword,    // 搜索关键字
        bkey: state.changeInformationOutputListFiltrate.bkey,     // 源下拉列表选项
        ckey: state.changeInformationOutputListFiltrate.ckey,     // 选中的类别标签
    }
}

export default connect(mapStateToProps)(InformationOutputListContainer);