...
/**
 * 资讯源实体类型定义
 * 严格对应数据库表 information_source 字段
 */
interface InformationSourceItem {
  bid: string;                  // 源id
  crawler_id: string;           // 爬虫id
  information_id: string | number; // 资讯id 主键
  title: string;                // 标题
...
export default function InformationSourcePage() {
  // 获取全局公共数据源、分类数据源
  ...
  const [dataSource, setDataSource] = useState<InformationSourceItem[]>([]);
  ...
  const [deleteRecord, setDeleteRecord] = useState<InformationSourceItem | null>(null);
  ...
  const fetchList = async () => {
    try {
      ...
      const res = await getInformationSourceApi(params);
   ...
   */
  const handleEditOpen = (record: InformationSourceItem) => {
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const res = await addInformationSourceApi(submitParams);
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const res = await updateInformationSourceApi(values.information_id, submitParams);
      const msg = res.data?.status?.message;
   ...
   * 打开删除确认弹窗
   */
  const handleDeleteOpen = (record: InformationSourceItem) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const res = await deleteInformationSourceApi(deleteRecord.information_id);
      const msg = res.data?.status?.message;
   ...
   */
  const columns: TableProps<InformationSourceItem>['columns'] = [
    {
      title: '序号',
      width: 70,
      align: 'center',
  ...
  return (
    <MainLayout>
      ...
      {/* 新增弹窗 */}
      <Modal open={addVisible} onCancel={() => setAddVisible(false)} title="新增资讯源" footer={null} width={700}>
      ...
      {/* 修改弹窗 */}
      <Modal open={editVisible} onCancel={() => setEditVisible(false)} title="修改资讯源" footer={null} width={700}>
        <Form form={editForm} layout="vertical" style={{ height: '70vh', overflowY: 'auto' }}>
          <Form.Item name="information_id" hidden><Input /></Form.Item>
