...
import {
    fetchGetAllInformationWeightSource,
    fetchDeleteInformationWeightSource,
    fetchUpdateInformationWeightSource,
    changeInformationWeightSourceFiltrate
} from '../action/InformationWeightSourceAction';
import { Layout, Divider, Popconfirm, Select, Tabs } from 'antd';
import moment from 'moment';
import SearchComponent from '../component/SearchComponent';
import TableComponent from '../component/TableComponent';
import { UpdateInformationWeightSourceComponent } from '../component/UpdateInformationWeightSourceComponent';

const { Content } = Layout;
const Option = Select.Option;
const TabPane = Tabs.TabPane;

class InformationWeightSourceContainer extends Component {
    constructor(props) {
        super(props);
        this.state = {
            updateModalVisible: false    // 更新权重源对话框显示状态
        }
        // 绑定回调方法
                            ...
                            render: (text, record) => {
                                ...
                                for (let key in dataObj) {
                                    ...
                                    if (key === 'weight_source_id') {
                                        var weightSourceId = dataObj[key];     // 本条记录的权重源id
                                    }
                                }
                                ...
                                function showUpdateModal() {
                                    // 通过回调方法，显示更新权重源对话框，并传递代表本条数据的对象
                                    handleShowUpdateModal(dataObj);
                                }

                                function deleteConfirm() {
                                    // 通过回调方法删除权重源
                                    handleDelete(weightSourceId);
                                }
    ...
    handleGet(keyword, pageNum, bkey, ckey) {
        ...
        // 5.通过props中的值发起请求
        dispatch(fetchGetAllInformationWeightSource(keyword, pageNum, bkey, ckey));
    }
    ...
    handleSourceChange(bkey) {
        ...
        dispatch(changeInformationWeightSourceFiltrate(this.props.keyword, bkey, this.props.ckey));
    ...
    handleCategoryChange(ckey) {
        const { dispatch } = this.props;
        // 1.更新store中对应数据,其他字段使用现有值
        dispatch(changeInformationWeightSourceFiltrate(this.props.keyword, this.props.bkey, ckey));
    ...
    handleSearchInputChange(text) {
        ...
        dispatch(changeInformationWeightSourceFiltrate(text, this.props.bkey, this.props.ckey));
    }
            ...
            <Content style={ {margin: '24px 16px', padding: '6px 24px', background: '#fff', minHeight: 615} }>
                <Tabs
                    ...
                    { categoryTabs.map(pane => <TabPane tab={ pane.tab } key={ pane.key } >
                        ...
                        <TableComponent rowKey='weight_source_id' tableData={ this.props.payload } onGet={ this.handleGet } loading={ this.props.isFetching } /> 
                    </TabPane>) }
                </Tabs>
                <UpdateInformationWeightSourceComponent 
...
// 对属性进行限定
InformationWeightSourceContainer.propTypes = {
...
function mapStateToProps(state) {
    return {
        payload: state.getInformationWeightSource.payload,    // 表格和分页渲染所需数据
        redisData: state.getInformationWeightSource.redisData,        // redis中的状态(status)和批次号(batch)数据
        isFetching: state.getInformationWeightSource.isFetching,    // 获取权重源数据加载态
        status: state.getInformationWeightSource.status,    // 获取权重源数据返回结果
        receiveAt: state.getInformationWeightSource.receiveAt,    // 获取权重源数据成功时间
        deleteStatus: state.deleteInformationWeightSource.status,    // 删除权重源返回结果
        isUpdating: state.updateInformationWeightSource.isUpdating,   // 更新权重源加载态
        updateStatus: state.updateInformationWeightSource.status,    // 更新权重源返回结果
        height: state.updateScreen.height,   // 屏幕高(浏览器可视区域)
        sourceOption: state.getMeta.sourceOption,   // 权重源源元数据
        categoryList: state.getMeta.categoryList,   // 类别元数据
        keyword: state.changeInformationWeightSourceFiltrate.keyword,    // 搜索关键字
        bkey: state.changeInformationWeightSourceFiltrate.bkey,     // 源下拉列表选项
        ckey: state.changeInformationWeightSourceFiltrate.ckey,     // 选中的类别标签
    }
}

export default connect(mapStateToProps)(InformationWeightSourceContainer);