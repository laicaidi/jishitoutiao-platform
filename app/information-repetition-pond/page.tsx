...
import {
  ...
  Modal, Form, Select, TableProps, Tag
} from 'antd';
...
/**
 * 滤重池实体类型定义
 * 对应数据库表 information_repetition_pond
 */
interface InformationRepetitionPondItem {
  repetition_pond_id: string;        // 滤重池id
...
export default function InformationRepetitionPondPage() {
  ...
  const [dataSource, setDataSource] = useState<InformationRepetitionPondItem[]>([]); // 列表数据
  ...
  const fetchList = async () => {
    try {
      ...
      // 请求接口
      const res = await getInformationRepetitionPondApi(params);
      // 赋值列表数据与总数
  ...
  const handleClearConfirm = async () => {
    try {
      ...
      // 调用清空接口
      const res = await clearInformationRepetitionPondApi();
      // 后台返回提示信息
   ...
   * 表格列配置
   */
  const columns: TableProps<InformationRepetitionPondItem>['columns'] = [
    {
      title: '序号',
      width: 70,
      align: 'center',
    ...
    { title: '滤重池ID', dataIndex: 'repetition_pond_id', width: 220 },
