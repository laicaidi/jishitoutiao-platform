...
import {
    fetchGetAllInformationIllegalityResult,
    fetchDeleteInformationIllegalityResult,
    fetchUpdateInformationIllegalityResult,
    changeInformationIllegalityResultFiltrate
...
import { UpdateInformationIllegalityResultComponent } from '../component/UpdateInformationIllegalityResultComponent';

const { Content } = Layout;
const Option = Select.Option;
const TabPane = Tabs.TabPane;

class InformationIllegalityResultContainer extends Component {
    constructor(props) {
        super(props);
        this.state = {
            updateModalVisible: false    // 更新滤非法结果对话框显示状态
        }
        // 绑定回调方法
                        ...
                        payloadObj[field].push({
                            ...
                            render: (text, record) => {
                                ...
                                for (let key in dataObj) {
                                    ...
                                    if (key === 'illegality_result_id') {
                                        var illegalityResultId = dataObj[key];     // 本条记录的滤非法结果id
                                    }
                                }
                                ...
                                function showUpdateModal() {
                                    // 通过回调方法，显示更新滤非法结果对话框，并传递代表本条数据的对象
                                    handleShowUpdateModal(dataObj);
                                }

                                function deleteConfirm() {
                                    // 通过回调方法删除滤非法结果
                                    handleDelete(illegalityResultId);
                                }
    ...
    handleGet(keyword, pageNum, bkey, ckey) {
        ...
        if (keyword !== null && keyword !== undefined) {        // 1.1从搜索框唤起
            // 更新props中的内容
            dispatch(changeInformationIllegalityResultFiltrate(keyword, this.props.bkey, this.props.ckey));
        ...
        // 5.通过props中的值发起请求
        dispatch(fetchGetAllInformationIllegalityResult(keyword, pageNum, bkey, ckey));
    }

    // 删除滤非法结果
    handleDelete(informationIllegalityResultId) {
        const { dispatch } = this.props;
        // 提交删除请求
        dispatch(fetchDeleteInformationIllegalityResult(informationIllegalityResultId, this.handleGet));
    }
    ...
    handleUpdate() {
        ...
        form.validateFields((err, values) => {
            ...
            for (let key in values) {
                ...
                if (key === 'illegality_result_id') {
                    var illegalityResultId = values[key];
            ...
            // 提交更新请求
            dispatch(fetchUpdateInformationIllegalityResult(illegalityResultId, formData, this.updateResult, this.handleGet));
        });
    }
    ...
    handleSourceChange(bkey) {
        ...
        dispatch(changeInformationIllegalityResultFiltrate(this.props.keyword, bkey, this.props.ckey));
    ...
    handleCategoryChange(ckey) {
        const { dispatch } = this.props;
        // 1.更新store中对应数据,其他字段使用现有值
        dispatch(changeInformationIllegalityResultFiltrate(this.props.keyword, this.props.bkey, ckey));
    ...
    handleSearchInputChange(text) {
        ...
        dispatch(changeInformationIllegalityResultFiltrate(text, this.props.bkey, this.props.ckey));
    }
    ...
    render() {
        ...
        if (redisData !== undefined && redisData !== null) {
            for (let key in redisData) {
                if (key === 'string:information_illegality_result:status') {
                    var status = redisData[key];
                }
                if (key === 'string:information_illegality_result:batch') {
            ...
            <Content style={ {margin: '24px 16px', padding: '6px 24px', background: '#fff', minHeight: 615} }>
                <Tabs
                    ...
                    { categoryTabs.map(pane => <TabPane tab={ pane.tab } key={ pane.key } >
                        ...
                        <TableComponent rowKey='illegality_result_id' tableData={ this.props.payload } onGet={ this.handleGet } loading={ this.props.isFetching } /> 
                    </TabPane>) }
                </Tabs>
                <UpdateInformationIllegalityResultComponent 
...
// 对属性进行限定
InformationIllegalityResultContainer.propTypes = {
...
function mapStateToProps(state) {
    return {
        payload: state.getInformationIllegalityResult.payload,    // 表格和分页渲染所需数据
        redisData: state.getInformationIllegalityResult.redisData,        // redis中的状态(status)和批次号(batch)数据
        isFetching: state.getInformationIllegalityResult.isFetching,    // 获取滤非法结果数据加载态
        status: state.getInformationIllegalityResult.status,    // 获取滤非法结果数据返回结果
        receiveAt: state.getInformationIllegalityResult.receiveAt,    // 获取滤非法结果数据成功时间
        deleteStatus: state.deleteInformationIllegalityResult.status,    // 删除滤非法结果返回结果
        isUpdating: state.updateInformationIllegalityResult.isUpdating,   // 更新滤非法结果加载态
        updateStatus: state.updateInformationIllegalityResult.status,    // 更新滤非法结果返回结果
        height: state.updateScreen.height,   // 屏幕高(浏览器可视区域)
        sourceOption: state.getMeta.sourceOption,   // 滤非法结果源元数据
        categoryList: state.getMeta.categoryList,   // 类别元数据
        keyword: state.changeInformationIllegalityResultFiltrate.keyword,    // 搜索关键字
        bkey: state.changeInformationIllegalityResultFiltrate.bkey,     // 源下拉列表选项
        ckey: state.changeInformationIllegalityResultFiltrate.ckey,     // 选中的类别标签
    }
}

export default connect(mapStateToProps)(InformationIllegalityResultContainer);