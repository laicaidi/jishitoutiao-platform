...
import {
  fetchGetAllCrawlerSet,
  fetchAddCrawlerSet,
  fetchDeleteCrawlerSet,
  fetchUpdateCrawlerSet,
  changeCrawlerSetFiltrate
} from '../action/CrawlerSetAction';
import { Layout, Button, Divider, Popconfirm  } from 'antd';
import SearchComponent from '../component/SearchComponent';
import TableComponent from '../component/TableComponent';
import { AddCrawlerSetComponent } from '../component/AddCrawlerSetComponent';
import { UpdateCrawlerSetComponent } from '../component/UpdateCrawlerSetComponent';

const { Content } = Layout;

class CrawlerSetContainer extends Component {
  constructor(props) {
    super(props);
    this.state = {
      addModalVisible: false,    // 新增爬虫设置对话框显示状态
      updateModalVisible: false    // 更新爬虫设置对话框显示状态
    }
    // 绑定回调方法
            ...
            payloadObj[field].push({
              ...
              render: (text, record) => {
                ...
                for (let key in dataObj) {
                  if (key === 'ckey') {
                      var msgCKey = dataObj[key];
                  } 
                  if (key === 'cvalue') {
                      var msgCValue = dataObj[key];
                  }  
                  if (key === 'sid') {        // 本条记录的sid
                      var sid = dataObj[key];
                  }
                ...
                const msgText = (
                  <div>
                      <h4>删除爬虫设置</h4>
                      <p>键: { msgCKey }</p>
                      <p>设置name: { msgCValue }</p>
                      <p>删除后不可恢复，是否确认？</p>
                  </div>
                );

                function showUpdateModel() {    
                  // 通过回调方法，显示更新爬虫设置对话框，并传递代表本条数据的对象
  ...
  handleGet(keyword, pageNum) {
    ...
    if (keyword !== undefined && keyword !== null) {    // 1.1搜索框唤起
      dispatch(changeCrawlerSetFiltrate(keyword));
    ...
    // 3.通过props中的值发起请求
    dispatch(fetchGetAllCrawlerSet(keyword, pageNum));
  }
  ...
  handleAdd() {
    ...
    form.validateFields((err, values) => {
      ...
      // 提交新增请求
      dispatch(fetchAddCrawlerSet(formData, this.addResult, this.handleGet));
    });
  }
  ...
  handleDelete(id) {
    const { dispatch } = this.props;
    // 提交删除请求
    dispatch(fetchDeleteCrawlerSet(id, this.handleGet));
  }
  ...
  handleUpdate() {
    ...
    form.validateFields((err, values) => {
      ...
      for (let key in values) {
        // bid作为url参数，其他字段通过PUT请求的body数据进行请求
        if (key === 'sid') {
          var sid = values[key];
        } else {
          formData[key] = values[key];
      ...
      // 提交更新请求
      dispatch(fetchUpdateCrawlerSet(sid, formData, this.updateResult, this.handleGet));
    });
  }
  ...
  handleSearchInputChange(text) {
    ...
    dispatch(changeCrawlerSetFiltrate(text));
  }
...
function mapStateToProps(state) {
  return {
      payload: state.getCrawlerSet.payload,    // 表格和分页渲染所需数据
      isFetching: state.getCrawlerSet.isFetching,    // 获取爬虫设置数据加载态
      status: state.getCrawlerSet.status,    // 获取爬虫设置数据返回结果
      receiveAt: state.getCrawlerSet.receiveAt,    // 获取爬虫设置数据成功时间
      isCreating: state.addCrawlerSet.isCreating,   // 新增爬虫设置加载态
      createStatus: state.addCrawlerSet.status,    //  新增爬虫设置返回结果(包含提示信息)
      deleteStatus: state.deleteCrawlerSet.status,    // 删除爬虫设置返回结果
      isUpdating: state.updateCrawlerSet.isUpdating,   // 更新爬虫设置加载态
      updateStatus: state.updateCrawlerSet.status,    // 更新爬虫设置返回结果
      height: state.updateScreen.height,   // 屏幕高(浏览器可视区域)
      keyword: state.changeCrawlerSetFiltrate.keyword    // 搜索关键字
  }
}

export default connect(mapStateToProps)(CrawlerSetContainer);