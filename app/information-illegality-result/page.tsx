...
/**
 * 滤非法结果实体类型定义
 * 严格对应数据库表 information_illegality_result 字段
 */
interface InformationIllegalityResultItem {
  illegality_result_id: string | number; // 滤非法结果id 主键
...
export default function InformationIllegalityResultPage() {
  // 获取全局公共数据源、分类数据源
  ...
  const [dataSource, setDataSource] = useState<InformationIllegalityResultItem[]>([]);
  ...
  const [deleteRecord, setDeleteRecord] = useState<InformationIllegalityResultItem | null>(null);
  ...
  const fetchList = async () => {
    try {
      ...
      const res = await getInformationIllegalityResultApi(params);
   ...
   */
  const handleEditOpen = (record: InformationIllegalityResultItem) => {
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const res = await addInformationIllegalityResultApi(submitParams);
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      const res = await updateInformationIllegalityResultApi(values.illegality_result_id, submitParams);
      const msg = res.data?.status?.message;
   ...
   * 打开删除确认弹窗
   */
  const handleDeleteOpen = (record: InformationIllegalityResultItem) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const res = await deleteInformationIllegalityResultApi(deleteRecord.illegality_result_id);
      const msg = res.data?.status?.message;
   ...
   * 表格列配置
   */
  const columns: TableProps<InformationIllegalityResultItem>['columns'] = [
    {
      title: '序号',
      width: 70,
      align: 'center',
    ...
    { title: '滤非法结果id', dataIndex: 'illegality_result_id', width: 220 },
    ...
    <MainLayout>
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <div>共计 {total} 条</div>
          <Pagination current={pageNum} total={total} pageSize={pageSize} onChange={setPageNum} />
        </div>
      </div>

      {/* 新增弹窗 */}
      <Modal open={addVisible} onCancel={() => setAddVisible(false)} title="新增滤非法结果" footer={null} width={700}>
      ...
      {/* 修改弹窗 */}
      <Modal open={editVisible} onCancel={() => setEditVisible(false)} title="修改滤非法结果" footer={null} width={700}>
        <Form form={editForm} layout="vertical" style={{ height: '70vh', overflowY: 'auto' }}>
          <Form.Item name="illegality_result_id" hidden><Input /></Form.Item>
      ...
      >
        {deleteRecord && (
          <div style={{ padding: '10px 0' }}>
            <p><strong>滤非法结果id:</strong> {deleteRecord.illegality_result_id}</p>
