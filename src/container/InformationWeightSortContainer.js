...
import {
    fetchGetAllInformationWeightSort,
    fetchDeleteInformationWeightSort,
    fetchUpdateInformationWeightSort,
    changeInformationWeightSortFiltrate
} from '../action/InformationWeightSortAction';
import { Layout, Divider, Popconfirm, Select, Tabs } from 'antd';
import moment from 'moment';
import SearchComponent from '../component/SearchComponent';
import TableComponent from '../component/TableComponent';
import { UpdateInformationWeightSortComponent } from '../component/UpdateInformationWeightSortComponent';

const { Content } = Layout;
const Option = Select.Option;
const TabPane = Tabs.TabPane;

class InformationWeightSortContainer extends Component {
    constructor(props) {
        super(props);
        this.state = {
                        ...
                        payloadObj[field].push({
                            ...
                            render: (text, record) => {
                                ...
                                for (let key in dataObj) {
                                    ...
                                    if (key === 'weight_sort_id') {
                                        var weightSortId = dataObj[key];     // 本条记录的得分数据id
                                    }
                                }
    ...
    handleGet(keyword, pageNum, bkey, ckey) {
        ...
        if (keyword !== null && keyword !== undefined) {        // 1.1从搜索框唤起
            // 更新props中的内容
            dispatch(changeInformationWeightSortFiltrate(keyword, this.props.bkey, this.props.ckey));
        ...
        // 5.通过props中的值发起请求
        dispatch(fetchGetAllInformationWeightSort(keyword, pageNum, bkey, ckey));
    }

    // 删除得分数据
    handleDelete(informationWeightSortId) {
        const { dispatch } = this.props;
        // 提交删除请求
        dispatch(fetchDeleteInformationWeightSort(informationWeightSortId, this.handleGet));
    }
    ...
    handleUpdate() {
        ...
        form.validateFields((err, values) => {
            ...
            // 提交更新请求
            dispatch(fetchUpdateInformationWeightSort(weightSortId, formData, this.updateResult, this.handleGet));
        });
    }
    ...
    // 源下拉列表更改时的回调
    handleSortChange(bkey) {
        const { dispatch } = this.props;
        // 1.更新store中对应数据,其他字段使用现有值
        dispatch(changeInformationWeightSortFiltrate(this.props.keyword, bkey, this.props.ckey));
    ...
    handleCategoryChange(ckey) {
        const { dispatch } = this.props;
        // 1.更新store中对应数据,其他字段使用现有值
        dispatch(changeInformationWeightSortFiltrate(this.props.keyword, this.props.bkey, ckey));
    ...
    handleSearchInputChange(text) {
        ...
        dispatch(changeInformationWeightSortFiltrate(text, this.props.bkey, this.props.ckey));
    }
    ...
    render() {
        ...
        if (redisData !== undefined && redisData !== null) {
            for (let key in redisData) {
                if (key === 'string:information_weight_sort:status') {
                    var status = redisData[key];
                }
                if (key === 'string:information_weight_sort:batch') {
            ...
            <Content style={ {margin: '24px 16px', padding: '6px 24px', background: '#fff', minHeight: 615} }>
                <Tabs
                    ...
                    { categoryTabs.map(pane => <TabPane tab={ pane.tab } key={ pane.key } >
                        ...
                        <TableComponent rowKey='weight_sort_id' tableData={ this.props.payload } onGet={ this.handleGet } loading={ this.props.isFetching } tag='InformationWeightSort' /> 
                    </TabPane>) }
                </Tabs>
                <UpdateInformationWeightSortComponent 
...
// 对属性进行限定
InformationWeightSortContainer.propTypes = {
...
function mapStateToProps(state) {
    return {
        payload: state.getInformationWeightSort.payload,    // 表格和分页渲染所需数据
        redisData: state.getInformationWeightSort.redisData,        // redis中的状态(status)和批次号(batch)数据
        isFetching: state.getInformationWeightSort.isFetching,    // 获取得分数据数据加载态
        status: state.getInformationWeightSort.status,    // 获取得分数据数据返回结果
        receiveAt: state.getInformationWeightSort.receiveAt,    // 获取得分数据数据成功时间
        deleteStatus: state.deleteInformationWeightSort.status,    // 删除得分数据返回结果
        isUpdating: state.updateInformationWeightSort.isUpdating,   // 更新得分数据加载态
        updateStatus: state.updateInformationWeightSort.status,    // 更新得分数据返回结果
        ...
        keyword: state.changeInformationWeightSortFiltrate.keyword,    // 搜索关键字
        bkey: state.changeInformationWeightSortFiltrate.bkey,     // 源下拉列表选项
        ckey: state.changeInformationWeightSortFiltrate.ckey,     // 选中的类别标签
    }
}

export default connect(mapStateToProps)(InformationWeightSortContainer);