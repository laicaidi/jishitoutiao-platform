...
class InformationWeightFiltrateContainer extends Component {
    ...
    handleUpdate() {
        ...
        form.validateFields((err, values) => {
            ...
            // 提交更新请求
            dispatch(fetchUpdateInformationWeightFiltrate(weightFiltrateId, formData));
            ...
            <Content style={ {margin: '24px 16px', padding: '6px 24px', background: '#fff', minHeight: 615} }>
                <Tabs
                    ...
                    { categoryTabs.map(pane => <TabPane tab={ pane.tab } key={ pane.key } >
                        ...
                        <TableComponent tableData={ this.props.payload } onGet={ this.handleGet } loading={ this.props.isFetching } tag='InformationWeightFiltrate' /> 
                    </TabPane>) }
                </Tabs>
