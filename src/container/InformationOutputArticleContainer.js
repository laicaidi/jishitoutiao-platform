...
import {
    fetchGetAllInformationOutputArticle,
    fetchDeleteInformationOutputArticle,
    fetchUpdateInformationOutputArticle,
    changeInformationOutputArticleFiltrate
...
import { UpdateInformationOutputArticleComponent } from '../component/UpdateInformationOutputArticleComponent';

const { Content } = Layout;
const Option = Select.Option;
const TabPane = Tabs.TabPane;

class InformationOutputArticleContainer extends Component {
    constructor(props) {
        super(props);
        this.state = {
            updateModalVisible: false    // 更新资讯内容对话框显示状态
        }
        // 绑定回调方法
                        ...
                        payloadObj[field].push({
                            ...
                            render: (text, record) => {
                                ...
                                for (let key in dataObj) {
                                    ...
                                    if (key === 'output_article_id') {
                                        var outputArticleId = dataObj[key];     // 本条记录的资讯内容id
                                    }
                                }
                                ...
                                function showUpdateModal() {
                                    // 通过回调方法，显示更新资讯内容对话框，并传递代表本条数据的对象
                                    handleShowUpdateModal(dataObj);
                                }

                                function deleteConfirm() {
                                    // 通过回调方法删除资讯内容
                                    handleDelete(outputArticleId);
                                }
    ...
    handleGet(keyword, pageNum, bkey, ckey) {
        ...
        if (keyword !== null && keyword !== undefined) {        // 1.1从搜索框唤起
            // 更新props中的内容
            dispatch(changeInformationOutputArticleFiltrate(keyword, this.props.bkey, this.props.ckey));
        ...
        // 5.通过props中的值发起请求
        dispatch(fetchGetAllInformationOutputArticle(keyword, pageNum, bkey, ckey));
    }

    // 删除资讯内容
    handleDelete(informationOutputArticleId) {
        const { dispatch } = this.props;
        // 提交删除请求
        dispatch(fetchDeleteInformationOutputArticle(informationOutputArticleId, this.handleGet));
    }
    ...
    handleUpdate() {
        ...
        form.validateFields((err, values) => {
            ...
            for (let key in values) {
                ...
                if (key === 'output_article_id') {
                    var outputArticleId = values[key];
            ...
            // 提交更新请求
            dispatch(fetchUpdateInformationOutputArticle(outputArticleId, formData, this.updateResult, this.handleGet));
        });
    }
    ...
    handleSourceChange(bkey) {
        ...
        dispatch(changeInformationOutputArticleFiltrate(this.props.keyword, bkey, this.props.ckey));
    ...
    handleCategoryChange(ckey) {
        const { dispatch } = this.props;
        // 1.更新store中对应数据,其他字段使用现有值
        dispatch(changeInformationOutputArticleFiltrate(this.props.keyword, this.props.bkey, ckey));
    ...
    handleSearchInputChange(text) {
        ...
        dispatch(changeInformationOutputArticleFiltrate(text, this.props.bkey, this.props.ckey));
    }
    ...
    render() {
        ...
        if (redisData !== undefined && redisData !== null) {
            for (let key in redisData) {
                if (key === 'string:information_output_article:status') {
                    var status = redisData[key];
                }
                if (key === 'string:information_output_article:batch') {
            ...
            <Content style={ {margin: '24px 16px', padding: '6px 24px', background: '#fff', minHeight: 615} }>
                <Tabs
                    ...
                    { categoryTabs.map(pane => <TabPane tab={ pane.tab } key={ pane.key } >
                        ...
                        <TableComponent rowKey='output_article_id' tableData={ this.props.payload } onGet={ this.handleGet } loading={ this.props.isFetching } /> 
                    </TabPane>) }
                </Tabs>
                <UpdateInformationOutputArticleComponent 
...
// 对属性进行限定
InformationOutputArticleContainer.propTypes = {
...
function mapStateToProps(state) {
    return {
        payload: state.getInformationOutputArticle.payload,    // 表格和分页渲染所需数据
        redisData: state.getInformationOutputArticle.redisData,        // redis中的状态(status)和批次号(batch)数据
        isFetching: state.getInformationOutputArticle.isFetching,    // 获取资讯内容数据加载态
        status: state.getInformationOutputArticle.status,    // 获取资讯内容数据返回结果
        receiveAt: state.getInformationOutputArticle.receiveAt,    // 获取资讯内容数据成功时间
        deleteStatus: state.deleteInformationOutputArticle.status,    // 删除资讯内容返回结果
        isUpdating: state.updateInformationOutputArticle.isUpdating,   // 更新资讯内容加载态
        updateStatus: state.updateInformationOutputArticle.status,    // 更新资讯内容返回结果
        height: state.updateScreen.height,   // 屏幕高(浏览器可视区域)
        sourceOption: state.getMeta.sourceOption,   // 资讯内容源元数据
        categoryList: state.getMeta.categoryList,   // 类别元数据
        keyword: state.changeInformationOutputArticleFiltrate.keyword,    // 搜索关键字
        bkey: state.changeInformationOutputArticleFiltrate.bkey,     // 源下拉列表选项
        ckey: state.changeInformationOutputArticleFiltrate.ckey,     // 选中的类别标签
    }
}

export default connect(mapStateToProps)(InformationOutputArticleContainer);